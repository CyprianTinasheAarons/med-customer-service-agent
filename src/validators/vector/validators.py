...
class IndexNameStructure(BaseModel):
    index_name: str = Field(
        description="Lower case name of the index you want to create")
...
class ExpectedNewData(BaseModel):
    new_info: Dict[str, str] = Field(
        description="Expected a pair key:value of question and answer.")

    @field_validator('new_info')
    def check_lowercase(cls, v):
        if set(v.keys()) != {'question', 'answer'}:
            raise ValueError(
                "The structure of the dictionary should be {'question':'...,' 'answer':'...'}")
        return v
